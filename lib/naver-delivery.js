...
const DELIVERY_BUNDLE_GROUPS_URL = `${NAVER_API_BASE}/v1/product-delivery-info/bundle-groups?page=1&size=100`;
...
function normalizeBundleGroup(item) {
  const id = Number(item?.id ?? item?.deliveryBundleGroupId ?? item?.groupId ?? 0);
  if (!id) return null;

  return {
    id,
    name: String(item?.name ?? item?.groupName ?? `묶음그룹 ${id}`).trim(),
    usable: item?.usable !== false && item?.useYn !== 'N',
    isBaseGroup: toBoolean(item?.baseGroup) || toBoolean(item?.defaultYn),
    raw: item,
  };
}

function scoreBundleGroup(item) {
  if (!item) return -1;
  let score = 0;
  if (item.usable) score += 100;
  if (item.isBaseGroup) score += 120;
  if (item.name.includes('기본')) score += 30;
  return score;
}
...
async function resolveDeliveryProfile(headers) {
  try {
    const [outboundRaw, addressesRaw, bundleGroupsRaw] = await Promise.all([
      fetchJson(OUTBOUND_LOCATIONS_URL, headers).catch(() => ({ items: [] })),
      fetchJson(ADDRESS_BOOKS_URL, headers),
      fetchJson(DELIVERY_BUNDLE_GROUPS_URL, headers).catch(() => ({ items: [] })),
    ]);
    ...
    const addressBooks = pickArray(addressesRaw)
      ...
      .filter(Boolean);
    const bundleGroups = pickArray(bundleGroupsRaw)
      .map(normalizeBundleGroup)
      .filter(Boolean);
    ...
    const selectedBundleGroup = chooseBest(bundleGroups, scoreBundleGroup);
    ...
    const profile = {
      ...
      deliveryBundleGroupId: selectedBundleGroup?.id || null,
      deliveryBundleGroupName: selectedBundleGroup?.name || '',
      deliveryBundleGroupUsable: !!selectedBundleGroup?.id,
    };
...
function buildDeliveryInfo(profile = {}) {
  ...
  if (profile.deliveryBundleGroupId != null && profile.deliveryBundleGroupId !== '') {
    ...
    if (Number.isFinite(groupId) && groupId > 0) {
      ...
      deliveryInfo.deliveryBundleGroup = { id: groupId };
    }
  } else {
