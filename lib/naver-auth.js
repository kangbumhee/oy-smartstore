...
function getDispatcher() {
  ...
  try {
    ...
    console.warn('[proxy] ProxyAgent 생성 실패:', e.message);
    return undefined;
  }
}
...
async function proxyFetch(url, options = {}) {
  ...
  const isNaverApi = url.includes('commerce.naver.com') || url.includes('api.commerce');

  if (dispatcher) {
    const maxRetries = isNaverApi ? 3 : 1;
    for (let i = 0; i < maxRetries; i++) {
      try {
        const opts = { ...options, dispatcher };
        return await fetch(url, opts);
      } catch (proxyErr) {
        console.warn(`[proxy] 프록시 실패 (${i + 1}/${maxRetries}):`, proxyErr.message);
        if (i < maxRetries - 1) await new Promise((r) => setTimeout(r, 1000 * (i + 1)));
      }
    }
    if (isNaverApi) {
      throw new Error('프록시를 통한 네이버 API 연결 실패. Fixie 프록시 상태를 확인하세요.');
    }
    console.warn('[proxy] 직접 연결 시도 (비-네이버)');
  }
...
async function getServerIp() {
  try {
    const r = await proxyFetch('https://api.ipify.org?format=json', { signal: AbortSignal.timeout(5000) });
