...
const App = {
  ...
  PROMPT_TEMPLATES: {
    thumbnail_closeup:
      'EXTREME TIGHT CLOSE-UP headshot for a Korean Olive Young beauty product thumbnail. The frame shows ONLY the face of a young beautiful Korean woman (early 20s, dewy glowing skin, soft gradient lip tint, natural eyebrows) — cropped VERY tightly from mid-forehead to chin, with the face filling AT LEAST 75-80% of the entire image. Almost NO background visible. Shot from a slightly low 3/4 angle. She holds or touches the product from the reference image delicately near her cheek — the product occupies about 15% of the frame and must match the EXACT packaging design from the reference image (same colors, logos, patterns, shape). The ENTIRE image is framed by a THICK SOLID COLOR BORDER (5-6% of image width) in ONE vibrant color chosen from: coral orange (#FF6B47), hot pink (#FF3B7A), lime green (#7EC845), or sunny yellow (#FFD600). The border must be uniform, opaque, and bold like a picture frame. Leave a subtle darkened semi-transparent rectangular area in the BOTTOM-LEFT corner (about 20% width, 10% height) as a text placement zone. Lighting: ultra-bright, perfectly even, shadowless beauty lighting. Skin looks luminous and poreless. Style: Korean Olive Young product thumbnail, clean commercial, magazine cover crop. MUST: face fills 80% of frame, thick color border visible on all 4 sides, product visible near face matching reference exactly. NO text, NO watermarks, NO full body, NO shoulders below collarbone. Photorealistic. Square 1:1 aspect ratio.',
    thumbnail_closeup_male:
      'EXTREME TIGHT CLOSE-UP headshot for a Korean Olive Young grooming product thumbnail. The frame shows ONLY the face of a young handsome Korean man (early-mid 20s, clear healthy skin, clean-shaven, defined jawline) — cropped VERY tightly from mid-forehead to chin, face filling AT LEAST 75-80% of the entire image. Almost NO background visible. 3/4 angle close-up shot. He confidently holds the product from the reference image near his jawline or cheek — product occupies about 15% of the frame and must match the EXACT packaging from the reference (same colors, logos, patterns, shape). ENTIRE image framed by a THICK SOLID COLOR BORDER (5-6% of image width) in ONE bold color from: navy blue (#1B3A5C), forest green (#2D5A3D), charcoal (#3D3D3D), or burnt orange (#CC5500). Border must be uniform, opaque, and prominent. Bottom-left corner has a subtle semi-transparent rectangular zone for text (about 20% width, 10% height). Ultra-bright, even, shadowless lighting. Skin looks fresh and healthy. Style: Korean Olive Young grooming thumbnail, clean commercial quality. MUST: face fills 80% of frame, thick color border on all 4 sides, product near face matching reference exactly. NO text, NO watermarks, NO full body, NO shoulders below collarbone. Photorealistic. Square 1:1 aspect ratio.',
  ...
  saveSettings() {
    ...
    s.geminiModel = document.getElementById('setting-model').value;
    s.storeName = document.getElementById('setting-store-name').value.trim();
